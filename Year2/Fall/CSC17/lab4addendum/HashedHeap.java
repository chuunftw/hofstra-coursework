...
public class HashedHeap<KT,VT extends Comparable<? super VT>> implements QuickHeap<KT,VT> {
  ...
  public Optional<VT> get(KT key) {
    ...
    if (size<=0 || index == null || Entries[index].val() == null ) return Optional.empty();
  
    return Optional.of(Entries[index].val());
...
public Optional<VT> find_n_modify(KT key, VT default_val, Function<? super VT, ? extends VT> modifier) {
  ...
  public Stream<KVPair<KT,VT>> priority_stream() {
    ...
    .flatMap(Optional::stream) // Convert Optional<KVPair> to Stream<KVPair>
    .limit(size());
  }
