...
def LLF_scheduling(tasks, hyperperiod):
    timeline = []
    deadline_misses = []
...
def plot_schedule(timeline, deadline_misses, tasks, hyperperiod):
    ...
    # Gantt chart plot
    plt.subplot(2, 1, 1)
    plt.title('LLF Scheduling - Gantt Chart')
    plt.xlabel('Time')
    plt.ylabel('Tasks')
...
tasks = [
    ...
    Task(2, 8, 3, 8),  # Task 2: Period=8ms, Execution Time=3ms, Deadline=8ms, Priority=2
...
timeline, deadline_misses = LLF_scheduling(tasks, hyperperiod)
