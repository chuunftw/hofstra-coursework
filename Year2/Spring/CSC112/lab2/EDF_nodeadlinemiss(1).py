...
def EDF_nodeadline_scheduling(tasks, hyperperiod):
    timeline = []
    deadline_misses = []
    ...
    for current_time in range(hyperperiod):
        ...
        if ready_tasks:
            ...
            current_task = min(ready_tasks, key=lambda t: t.deadline)
...
def plot_schedule(timeline, deadline_misses, tasks, hyperperiod):
    ...
    sorted_tasks = sorted(tasks, key=lambda x: -x.deadline)  # Descending order of priority

    # Gantt chart plot
    plt.subplot(2, 1, 1)
    plt.title('EDF No Deadline - Gantt Chart')
    plt.xlabel('Time')
    plt.ylabel('Tasks')
...
tasks = [
    Task(1, 6, 2, 6, 1),  # Task 1: Period=6ms, Execution Time=2ms, Deadline=6ms, Priority=3 (highest)
    Task(2, 12, 3, 12, 2),  # Task 2: Period=8ms, Execution Time=3ms, Deadline=8ms, Priority=2
    Task(3, 12, 3, 12,3 )  # Task 3: Period=12ms, Execution Time=3ms, Deadline=12ms, Priority=1 (lowest)
]
...
timeline, deadline_misses = EDF_nodeadline_scheduling(tasks, hyperperiod)
